...
while True:  # nii kaua, kui tsükkel on tõene,
    ...
    for syndmus in pygame.event.get():  # sükkli muutujale omistatakse kõik pygame.event.get() väärtused
        ...
        elif syndmus.type == pygame.KEYDOWN:  # kui sündmuse tüüp on klahvi vajutus, siis
        ...
        elif syndmus.type == pygame.KEYUP:  # kui sündmuse tüüp on klahvi lahti laskmine, siis
    ...
    if pall_Y > alus_Y - alus_pilt.get_rect().height and pall_X > alus_X and pall_X < alus_X + alus_pilt.get_rect().width:  # kui palli y kordinaat on suurem kui aluse y kordinaat ja palli x kordinaat jääb aluse laiuse vahemikku, siis
